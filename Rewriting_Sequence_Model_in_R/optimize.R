optimize <- function(X, Y, a_prev, parameters, learning_rate = 0.005) {
        ...
        #list("loss" = loss, "gradients" = gradients, "a_Final" = a[length(X)], "parameters" = parameters )
        list("loss" = loss, "gradients" = gradients, "a_Final" = a[(length(X)+1)], "parameters" = parameters )
        #list("loss" = loss, "gradients" = gradients, "a_Final" = a[length(X)])
                              #Note! in python this is a[len(X)-1] as a starts from -1 to len(X). len(a) = len(X)+ 1
